...
func GetUserInfoDbConfig(env string) *mysql.DbConfig {
	curDbConfig = mysql.NewDbConfig()
	switch {
	case strings.Contains(env, "pro"):
		curDbConfig.DbName = "qy_user_info_prod"
	case strings.Contains(env, "test"):
		curDbConfig.DbName = "qy_user_info_test"
	default:
		curDbConfig.DbName = "qy_user_info_dev"
	}
	return curDbConfig
}
...
func GetDb() *gorm.DB {
	if curDbConfig == nil {
		panic("请先初始化数据库配置，调用：GetUserInfoDbConfig方法")
	}
